							...
							<?php
								mysql_connect("mysql.neq3.com","u148570980_rumi","rr12345");
         						mysql_select_db("u148570980_tour");
		       					...
		       					if(isset($_POST['package_no'])&&isset($_POST['from'])&&isset($_POST['to'])&&isset($_POST['stay'])&&isset($_POST['cost']))
								    {
											$package = $_POST['package_no'];
											$from = $_POST['from'];
											$to = $_POST['to'];
											$stay = $_POST['stay'];
											$cost = $_POST['cost'];
											
											$query = mysql_query("select * from packages  where package_no = $package ");
											
		                                    if(mysql_num_rows($query)> 0)     die(" The PACKAGE NUMBER  IS CONFLICTED ,YOU HAVE TO GO BACK        <----------- ");
												
											else { 
											      $insert = mysql_query("insert into packages values($package,'$from','$to','$stay','$cost')");
										    }
									}
							?>	

                                <h4 align="center"> <p>CREATE NEW PACKAGE  SUCCESSFULLY,</P>  <P> GO TO UPDATE PACKAGES PAGE  </P>
