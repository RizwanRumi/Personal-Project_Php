					...
					<div id="content1">
					  <table  height="150" width="0"> </table>
						...
						<?php
							...
							if( isset($_POST['hotel_no_del'] )&& isset($_POST['hotel_del']) )
               				  {
							    $hotel_no_del = $_POST['hotel_no_del'];
				            	$hotel_del = $_POST['hotel_del'];
															
								$query = mysql_query("select * from hotel_cost_list WHERE Hotel_No = $hotel_no_del and  Name='$hotel_del' ");
														
							    if( ! (mysql_num_rows($query)  ) )  die(" THE HOTEL NUMBER OR NAME  IS CONFLICTED ,YOU HAVE TO GO BACK        <----------- ");
								else
				       				$delete_selected = mysql_query("delete from hotel_cost_list WHERE  Hotel_No = $hotel_no_del and  Name='$hotel_del' ");
				 			  }
                        ?>

                              <h4 align="center">DELETE HOTEL  SUCCESSFULLY, GO TO UPDATE PACKAGES PAGE 
