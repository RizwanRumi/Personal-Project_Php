					  ...
					  <table  height="150" width="0"> </table>
						...
						<?php
						    ...
						    mysql_select_db("u148570980_tour");
							 ...
							 if(isset($_POST['package_no_del'])) 
							 {
								$package1 = $_POST['package_no_del'];
										
								$query = mysql_query("select * from packages  where package_no = $package1 ");
								
								  if( ! (mysql_num_rows($query)  ) )     die(" The PACKAGE NUMBER  IS CONFLICTED ,YOU HAVE TO GO BACK        <----------- ");
								  
								  else
							                $delete_selected = mysql_query("delete from packages WHERE package_no = $package1 ");
							 } 
						?>	

                              <h4 align="center"> DELETE SELECTED PACKAGE , GO TO UPDATE PACKAGES PAGE 
