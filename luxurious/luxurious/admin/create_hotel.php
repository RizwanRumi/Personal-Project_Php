					...
					<div id="content1">
					  <table  height="150" width="0"> </table>
							...
							<?php
		    				    ...
		    				    if(isset($_POST['hotel_no']) && isset($_POST['hotel_name'])&&isset($_POST['hotel_classic'])&&isset($_POST['hotel_luxury'])&&isset($_POST['hotel_place']))
                         		   {
								        $hotel_no =$_POST['hotel_no']; 
								    	$hotel = $_POST['hotel_name'];
										$h_classic = $_POST['hotel_classic'];
										$h_luxury = $_POST['hotel_luxury'];
										$h_place = $_POST['hotel_place'];
														
										$query = mysql_query("select * from hotel_cost_list WHERE Hotel_No = $hotel_no ");
														
									    if(  (mysql_num_rows($query) > 0 ) )      die(" THE HOTEL NUMBER  IS CONFLICTED ,YOU HAVE TO GO BACK        <----------- ");
										else
										    $insert = mysql_query("insert into hotel_cost_list values($hotel_no,'$hotel','$h_classic','$h_luxury','$h_place')");
								   }
                                ?>

                              <h4 align="center"> CREATE NEW TRANSPORT SUCCESSFULLY, GO TO UPDATE PACKAGES PAGE 
